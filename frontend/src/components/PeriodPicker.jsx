...
const PRESET_KEYS = ['today', 'yesterday', 'week', 'month', 'lastMonth', 'quarter', 'year', 'lastYear', 'all'];
...
export default function PeriodPicker() {
  ...
  const labelFor = (preset, from, to) => {
    if (preset === 'all') return t('calendar.preset.all');
    if (PRESET_KEYS.includes(preset)) return t(`calendar.preset.${preset}`);
    if (from && to) return `${fmtD(from)} — ${fmtD(to)}`;
    if (from) return t('calendar.fromDate', { date: fmtD(from) });
    if (to)   return t('calendar.toDate', { date: fmtD(to) });
