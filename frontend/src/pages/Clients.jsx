...
// DB status qiymatini (o'zbekcha) tarjima kalitiga moslaydi — qiymatlar saqlanadi, faqat label tarjima qilinadi
const STATUS_KEY = {
  'Yangi': 'new', 'Faol': 'active', 'Kutilmoqda': 'pending',
  "Muddati o'tgan": 'overdue', "O'chirilgan": 'deleted',
};
      ...
      >
        <div className="flex flex-wrap gap-1">
          {selectedList.length === 0 ? (
            <span style={{ color: 'var(--text-3)' }}>{t('clients.sellersSelect')}</span>
          ) : (
            selectedList.map(s => (
...
export default function Clients({ user }) {
  ...
  const validate = () => {
    const e={};
    if(!form.name.trim()) e.name=t('clients.val.nameRequired');
    const inn=rawINN(form.inn);
    if(inn&&inn.length!==9) e.inn=t('clients.val.inn');
    if(form.account&&form.account.replace(/\D/g,'').length!==20) e.account=t('clients.val.account');
    if(form.mfo&&form.mfo.replace(/\D/g,'').length!==5) e.mfo=t('clients.val.mfo');
    const phone=rawPhone(phoneInput);
    if(phoneInput&&phone.replace(/\D/g,'').replace(/^998/,'').length!==9) e.phone=t('clients.val.phone');
    return e;
  };
  ...
  const handleCopy = c => {
    setForm({ name:(c.name||'') + t('clients.copySuffix'), inn:fmtINN(c.inn||''), phone:c.phone||'', director:c.director||'', address:c.address||'', category:c.category||'', status:c.status||'Yangi', account:c.account||'', mfo:c.mfo||'', bank:c.bank||'', seller:c.seller||'', country:c.country||'' });
    setPhoneInput(fmtPhone(c.phone||''));
  ...
  const handleSave = async e => {
    ...
    try {
      ...
      toast.success(modal==='add' ? t('clients.toast.added') : t('clients.toast.updated'));
  ...
  const handleDelete = async () => {
    try { await api.delete(`/api/clients/${delId}`); setDelId(null); fetchClients(); toast.success(t('common.deleted')); }
    catch { /* interceptor shows toast */ }
  };

  const handleRestore = async (rec) => {
    try { await api.post(`/api/clients/${rec.id}/restore`); toast.success(t('common.restored')); fetchClients(); }
    catch { /* interceptor toast */ }
  };
  const handleHardDelete = async (rec) => {
    if (!window.confirm(t('common.hardDeleteConfirm'))) return;
    try { await api.delete(`/api/clients/${rec.id}/hard`); toast.success(t('common.hardDeleted')); fetchClients(); }
    catch { /* interceptor toast */ }
  };
  ...
  const handleContractSave = async (e) => {
    if (e) e.preventDefault();
    if (!contractForm.number.trim()) { toast.error(t('clients.contract.numberRequired')); return; }
    if (!contractForm.date)          { toast.error(t('clients.contract.dateRequired')); return; }
    if (!contractForm.totalValue || parseFloat(contractForm.totalValue) < 0) { toast.error(t('clients.contract.sumRequired')); return; }
    if (!contractForm.seller)        { toast.error(t('clients.contract.sellerRequired')); return; }
    ...
    try {
      if (contractModal === 'edit') {
        ...
        toast.success(t('clients.contract.updated'));
      } else {
        await api.post('/api/contracts', payload);
        toast.success(t('clients.contract.added'));
      }
      closeContractModal();
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6 animate-in">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          <div>
            <h2 className="text-lg font-semibold text-[var(--text)]">{t('clients.title')}</h2>
            <p className="text-xs text-[var(--text-3)] mt-0.5">{t('clients.count', { count: total })}</p>
          </div>
          {canCreate && (
            <button onClick={openAdd} className="px-3 py-1.5 btn-primary rounded-lg text-xs font-medium transition flex items-center gap-2 shadow-sm">
              <Plus size={16} strokeWidth={2.2}/> {t('common.newClient')}
            </button>
          )}
        </div>
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="md:col-span-2">
                <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">{t('clients.f.name')} *</label>
                <input autoFocus type="text" value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} className={inp('name')} placeholder={t('clients.ph.name')}/>
                {errors.name&&<p className="text-[10px] text-red-500 mt-0.5">{errors.name}</p>}
              </div>
              <div>
                <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">{t('clients.f.inn')}</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">{t('clients.f.director')}</label>
                <input type="text" value={form.director} onChange={e=>setForm(f=>({...f,director:e.target.value}))} className={inp('director')} placeholder={t('clients.ph.director')}/>
              </div>
              <div>
                <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">{t('clients.f.phone')}</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">{t('clients.f.status')}</label>
                <select value={form.status} onChange={e=>setForm(f=>({...f,status:e.target.value}))} className={inp('status')}>
                  <option value="Yangi">{t('clients.status.new')}</option><option value="Faol">{t('clients.status.active')}</option><option value="Kutilmoqda">{t('clients.status.pending')}</option><option value="Muddati o'tgan">{t('clients.status.overdue')}</option>
                </select>
              </div>
              <div>
                <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">{t('clients.f.category')}</label>
                <input type="text" value={form.category} onChange={e=>setForm(f=>({...f,category:e.target.value}))} className={inp('category')} placeholder={t('clients.ph.category')}/>
              </div>
              <div>
                <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">{t('clients.f.country')}</label>
                <input type="text" value={form.country} onChange={e=>setForm(f=>({...f,country:e.target.value}))} className={inp('country')} placeholder={t('clients.ph.country')}/>
              </div>
              <div className="md:col-span-2">
                <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">{t('clients.f.address')}</label>
                <input type="text" value={form.address} onChange={e=>setForm(f=>({...f,address:e.target.value}))} className={inp('address')} placeholder={t('clients.ph.address')}/>
              </div>
              <div>
                <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">{t('clients.f.account')}</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">{t('clients.f.mfo')}</label>
              ...
              <div className="md:col-span-2">
                <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">{t('clients.f.bank')}</label>
                <input type="text" value={form.bank} onChange={e=>setForm(f=>({...f,bank:e.target.value}))} className={inp('bank')} placeholder={t('clients.ph.bank')}/>
              </div>
              <div className="md:col-span-2">
                <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">{t('clients.f.seller')}</label>
      ...
      <div className="mini-card p-0">
        <div className="px-5 py-3 border-b border-[var(--border)] bg-[var(--surface-2)] flex items-center gap-3">
          <div className="relative flex-1 max-w-xs">
            ...
            <input type="text" placeholder={t('clients.searchPlaceholder')} {...searchInput}
          ...
          {search&&<span className="text-xs text-[var(--text-3)]">{t('clients.results', { count: total })}</span>}
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-[var(--border)]">
                {[['name',t('clients.th.client')],['inn',t('clients.th.inn')],['phone',t('clients.th.phone')],['seller',t('clients.th.seller')],['status',t('clients.th.status')],['debt',t('clients.th.debt')]].map(([col,label])=>(
                ...
                <th className="px-5 py-3 text-[10.5px] font-semibold text-[var(--text-2)] uppercase tracking-wider whitespace-nowrap">{t('common.whoWhen')}</th>
            ...
            <tbody>
              {loading ? [...Array(5)].map((_,i)=>(
                ...
                <tr><td colSpan={COLS} className="px-5 py-12 text-center">
                  {search ? (
                    <span className="text-[var(--text-3)] text-xs">{t('common.notFound')}</span>
                  ) : (
                    <div className="flex flex-col items-center gap-3">
                      <span className="text-[var(--text-3)] text-xs">{t('clients.empty')}</span>
                      {canCreate && (
                        <button onClick={openAdd} className="px-3 py-1.5 btn-primary rounded-lg text-xs font-medium inline-flex items-center gap-2 shadow-sm">
                          <Plus size={15} strokeWidth={2.2}/> {t('clients.addFirst')}
                        </button>
                      )}
                    </div>
                  )}
                ...
                <Fragment key={c.id}>
                  <tr onClick={()=>toggleExpand(c.id)} className={`hover:bg-[var(--surface-2)] border-b border-[var(--border)] transition-colors group cursor-pointer ${expanded===c.id?'bg-[var(--surface-2)]':''}${c.deletedAt?' opacity-60 bg-red-50 dark:bg-red-950/20':''}`}>
                    ...
                    <td className="px-5 py-2.5">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold border ${SC[c.status]||SC['Yangi']}`}>{t('clients.status.' + (STATUS_KEY[c.status] || 'new'))}</span>
                    </td>
                      ...
                      <div className="flex items-center justify-end gap-1">
                        {c.deletedAt ? (
                          <>
                            ...
                            {canHardDelete && (
                              <>
                                <button onClick={()=>handleRestore(c)} className="text-[10px] text-[var(--accent)] font-semibold hover:underline px-1" title={t('common.restore')}>{t('common.restore')}</button>
                                <button onClick={()=>handleHardDelete(c)} className="p-1 text-red-500 hover:text-red-700 hover:bg-red-50 rounded transition" title={t('common.hardDelete')}><Trash2 size={15} strokeWidth={1.8}/></button>
                              </>
                            )}
                          </>
                        ) : (
                          <>
                            <button onClick={()=>handleCopy(c)} className="p-1 text-[var(--text-3)] hover:text-[var(--accent)] hover:bg-[var(--surface-2)] rounded transition opacity-0 group-hover:opacity-100" title={t('common.copy')}><Copy size={15} strokeWidth={1.8}/></button>
                            {canUpdate && (
                              <button onClick={()=>openEdit(c)} className="p-1 text-[var(--text-3)] hover:text-[var(--accent)] hover:bg-[var(--surface-2)] rounded transition opacity-0 group-hover:opacity-100" title={t('common.edit')}><Edit2 size={15} strokeWidth={1.8}/></button>
                            )}
                            {canDelete && (
                              <button onClick={()=>setDelId(c.id)} className="p-1 text-[var(--text-3)] hover:text-red-500 hover:bg-red-50 rounded transition opacity-0 group-hover:opacity-100" title={t('common.delete')}><Trash2 size={15} strokeWidth={1.8}/></button>
                            )}
                          </>
                        )}
                      </div>
                      ...
                      <td colSpan={COLS} className="px-5 py-3 border-b border-[var(--border)]">
                        <div className="grid grid-cols-3 gap-6 animate-in">
                          <div className="col-span-2 space-y-3">
                            <p className="text-[10px] font-semibold text-[var(--text-3)] uppercase tracking-wider mb-2">{t('clients.requisites')}</p>
                            <div className="grid grid-cols-2 gap-x-6 gap-y-1.5 text-xs">
                              {[[t('clients.f.inn'),fmtINN(c.inn||'')],[t('clients.f.phone'),fmtPhone(c.phone||'')],[t('clients.f.address'),c.address],[t('clients.f.category'),c.category],[t('clients.f.account'),c.account],[t('clients.f.mfo'),c.mfo],[t('clients.f.director'),c.director],[t('clients.f.seller'),c.seller]].map(([label,val])=>val?(
                            ...
                            <div className="flex items-center gap-2 mb-2">
                              {canCreateContract && (
                                <button onClick={() => openContractAdd(c.id)}
                                  ...
                                  <Plus size={11}/> {t('clients.contract.add')}
                                </button>
                              )}
                              <span className="text-[10px] font-semibold text-[var(--text-3)] uppercase tracking-wider">{t('clients.contract.heading')}</span>
                            </div>
                            {!expandContracts[c.id] ? (
                              <p className="text-xs text-[var(--text-3)]">{t('common.loading')}</p>
                            ) : expandContracts[c.id].length === 0 ? (
                              <p className="text-xs text-[var(--text-3)]">{t('clients.contract.none')}</p>
                            ) : (
                              <div className="space-y-1.5">
                                {expandContracts[c.id].map(ct => (
                                  <div key={ct.id} className="flex items-center justify-between text-xs bg-[var(--surface)] border border-[var(--border)] rounded-lg px-2.5 py-1.5 group/ct shadow-sm">
                                    ...
                                    <span className="text-[var(--text-3)] font-mono text-[10px]">{fmtDate(ct.date)}</span>
        ...
        <div className="modal-overlay">
          <div className="modal-card w-full max-w-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-[var(--border)] flex justify-between items-center">
              <h3 className="text-lg font-bold text-[var(--text)]">{t('clients.editTitle')}</h3>
              <div className="flex items-center gap-3">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-[var(--text-2)] mb-1">{t('clients.f.name')} *</label>
                  <input autoFocus type="text" value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} className={inp('name')} placeholder={t('clients.ph.name')}/>
                  {errors.name&&<p className="text-xs text-red-500 mt-1">{errors.name}</p>}
                </div>
                <div>
                  <label className="block text-sm font-medium text-[var(--text-2)] mb-1">{t('clients.f.inn')} <span className="text-[var(--text-3)] font-normal">{t('clients.hint.inn')}</span></label>
                  <input type="text" value={form.inn} onChange={e=>setForm(f=>({...f,inn:fmtINN(e.target.value)}))} className={inp('inn')} placeholder="123 456 789"/>
                  {errors.inn&&<p className="text-xs text-red-500 mt-1">{errors.inn}</p>}
                </div>
                <div>
                  <label className="block text-sm font-medium text-[var(--text-2)] mb-1">{t('clients.f.director')}</label>
                  <input type="text" value={form.director} onChange={e=>setForm(f=>({...f,director:e.target.value}))} className={inp('director')} placeholder={t('clients.ph.director')}/>
                </div>
                <div>
                  <label className="block text-sm font-medium text-[var(--text-2)] mb-1">{t('clients.f.phone')} <span className="text-[var(--text-3)] font-normal">{t('clients.hint.phone')}</span></label>
                  <div className="relative">
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--text-2)] mb-1">{t('clients.f.status')}</label>
                  <select value={form.status} onChange={e=>setForm(f=>({...f,status:e.target.value}))} className={inp('status')}>
                    <option value="Yangi">{t('clients.status.new')}</option><option value="Faol">{t('clients.status.active')}</option><option value="Kutilmoqda">{t('clients.status.pending')}</option><option value="Muddati o'tgan">{t('clients.status.overdue')}</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-[var(--text-2)] mb-1">{t('clients.f.category')}</label>
                  <input type="text" value={form.category} onChange={e=>setForm(f=>({...f,category:e.target.value}))} className={inp('category')} placeholder={t('clients.ph.category')}/>
                </div>
                <div>
                  <label className="block text-sm font-medium text-[var(--text-2)] mb-1">{t('clients.f.country')}</label>
                  <input type="text" value={form.country} onChange={e=>setForm(f=>({...f,country:e.target.value}))} className={inp('country')} placeholder={t('clients.ph.country')}/>
                </div>
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-[var(--text-2)] mb-1">{t('clients.f.address')}</label>
                  <input type="text" value={form.address} onChange={e=>setForm(f=>({...f,address:e.target.value}))} className={inp('address')} placeholder={t('clients.ph.address')}/>
                </div>
                <div>
                  <label className="block text-sm font-medium text-[var(--text-2)] mb-1">{t('clients.f.account')} <span className="text-[var(--text-3)] font-normal">{t('clients.hint.account')}</span></label>
                  <input type="text" value={form.account} onChange={e=>setForm(f=>({...f,account:e.target.value.replace(/\D/g,'').slice(0,20)}))} className={inp('account')} placeholder="20200000000000000000"/>
                  {errors.account&&<p className="text-xs text-red-500 mt-1">{errors.account}</p>}
                </div>
                <div>
                  <label className="block text-sm font-medium text-[var(--text-2)] mb-1">{t('clients.f.mfo')} <span className="text-[var(--text-3)] font-normal">{t('clients.hint.mfo')}</span></label>
                  <input type="text" value={form.mfo} onChange={e=>setForm(f=>({...f,mfo:e.target.value.replace(/\D/g,'').slice(0,5)}))} className={inp('mfo')} placeholder="01234"/>
                  {errors.mfo&&<p className="text-xs text-red-500 mt-1">{errors.mfo}</p>}
                </div>
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-[var(--text-2)] mb-1">{t('clients.f.bank')}</label>
                  <input type="text" value={form.bank} onChange={e=>setForm(f=>({...f,bank:e.target.value}))} className={inp('bank')} placeholder={t('clients.ph.bank')}/>
                </div>
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-[var(--text-2)] mb-1">{t('clients.f.seller')}</label>
                  <MultiSellerSelect sellers={sellers} value={form.seller} onChange={v=>setForm(f=>({...f,seller:v}))}/>
                </div>
              </div>
      ...
      {delId&&(
        <div className="modal-overlay">
          <div className="modal-card w-full max-w-sm p-6 text-center">
            ...
            <h3 className="text-lg font-bold text-[var(--text)] mb-2">{t('clients.deleteTitle')}</h3>
            <p className="text-sm text-[var(--text-3)] mb-6">{t('common.cannotUndo')}</p>
          ...
          <div className="modal-card w-full max-w-md">
            <div className="px-6 py-4 border-b border-[var(--border)] flex justify-between items-center">
              <h3 className="text-lg font-bold text-[var(--text)] flex items-center gap-2">
                ...
                {contractModal === 'edit' ? t('clients.contract.editTitle') : t('clients.contract.addTitle')}
              </h3>
            ...
            <form onSubmit={handleContractSave} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-[var(--text-2)] mb-1">{t('clients.contract.number')} *</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--text-2)] mb-1">{t('clients.contract.sum')} *</label>
              ...
              <div>
                ...
                <select value={contractForm.seller} onChange={e => setContractForm(f => ({ ...f, seller: e.target.value }))}
                  ...
                  <option value="">{t('clients.contract.selectSeller')}</option>
                ...
                {contractClientId && !(clients.find(c => c.id === contractClientId)?.seller || '').trim() && (
                  <p className="text-[11px] text-red-500 mt-1">{t('clients.contract.noSellers')}</p>
                )}
              </div>
              <div>
                <label className="block text-sm font-medium text-[var(--text-2)] mb-1">{t('clients.contract.currency')}</label>
                <select value={contractForm.currency} onChange={e => setContractForm(f => ({ ...f, currency: e.target.value }))}
                  className="w-full px-3 py-2 border border-[var(--border)] rounded-md text-sm focus:ring-2 focus:ring-[var(--accent)] outline-none transition bg-[var(--surface)]">
                  <option value="UZS">{t('clients.contract.curUzs')}</option>
                  <option value="USD">{t('clients.contract.curUsd')}</option>
                </select>
              </div>
              <div className="pt-4 border-t border-[var(--border)] flex justify-end gap-3">
                <button type="button" onClick={closeContractModal} className="px-4 py-2 text-sm font-medium text-[var(--text-2)] hover:bg-[var(--surface-2)] rounded-md transition">{t('common.cancel')}</button>
                <button type="submit" disabled={contractSaving} className="px-5 py-2 btn-primary disabled:opacity-60 text-sm font-medium rounded-md transition shadow-sm">
                  {contractSaving ? t('common.saving') : t('common.save')}
                </button>
              </div>
            </form>
          </div>
      ...
      {delContractId && (
        <div className="modal-overlay">
          <div className="modal-card w-full max-w-sm p-6 text-center">
            <div className="w-12 h-12 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4"><Trash2 size={22} className="text-red-600"/></div>
            <h3 className="text-lg font-bold text-[var(--text)] mb-2">{t('clients.contract.deleteTitle')}</h3>
            <p className="text-sm text-[var(--text-3)] mb-6">{t('clients.contract.deleteWarn')}</p>
            <div className="flex gap-3">
              <button onClick={() => setDelContractId(null)} className="flex-1 px-4 py-2 text-sm font-medium border border-[var(--border)] rounded-md hover:bg-[var(--surface-2)] transition">{t('common.cancel')}</button>
              <button onClick={handleContractDelete} className="flex-1 px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-md transition">{t('common.delete')}</button>
            </div>
          </div>
        </div>
      )}
